...
const { mungeTrail } = require('../utils.js');
...
app.get('/reviews', async(req, res) => {
  try {
    ...
    // https://api.yelp.com/v3/businesses/search?latitude=45.5202471&longitude=-122.6741949
    
    const response = await request.get(URL).set('Authorization', `Bearer ${process.env.YELP}`);
